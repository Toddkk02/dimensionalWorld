...
void InitPortalSystem(PortalSystem *ps)
{
    ...
    {
        const char *path = FileExists("assets/models/portalgun.obj") ? "assets/models/portalgun.obj" : "assets/models/portalgun.glb";
...
void UpdatePortalSystem(PortalSystem *ps, Camera3D camera, World *world, DimensionManager *dimManager, float deltaTime)
{
    ...
    if (wheel != 0.0f)
    {
        ...
        if (wheel < 0)
            ps->currentDimensionID = (ps->currentDimensionID + 1) % count;
        else
            ps->currentDimensionID = (ps->currentDimensionID - 1 + count) % count;
    }
